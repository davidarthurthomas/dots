...
**Structure-insensitive.** A good test doesn't care how you wrote the code, only what the code does. If you rename a private helper and a test breaks, that test was watching your typing, not your behavior. Test what a caller can see: what goes in, what comes out, what changes in the world.
...
**Fast and deterministic.** If the suite is slow, you stop running it, and a test you don't run is worthless. If a test fails at random, you learn to shrug at red, and now green means nothing either. Same inputs, same answer, whatever order you run things in.

**Rooted in the functional core.** The core is a joy to test. Hand it values, check the values it hands back, cover every edge and every failure. Keep the shell thin and a few end-to-end tests will cover it. Don't write a pile of little tests for plumbing just to move a number.

**Sparing with mocks.** Reach for real objects first. Most of the time, once you've split the core from the shell, you don't need a mock at all, because the core is just values in and values out. When you do need one, put it at an edge you own: the clock, the network, the disk. Don't mock someone else's library from the inside. And if a test is mostly mocks, you're testing your guesses, not your code.

**Readable as a bug report.** When a test fails, it should read like a bug report: here's what you expected, here's what you got. Set things up, do the thing, check the result, in that order, with the check at the end where you can see it. No logic in the test.
