...
**Server Components are the default; the client is the opt-in.** Comeau's rule is that "all components are assumed to be Server Components by default. We have to 'opt in' for Client Components." The reason to opt in is real interactivity, not habit: Server Components "cannot use interactive APIs like `useState`", so state, effects, event handlers, and browser APIs are what force a component to the client. Everything else stays on the server, out of the bundle and close to the data.
...
**Only serializable values cross the boundary.** Props "passed to Client Components need to be serializable by React"; an argument to a server function travels as "a serialized copy". Abramov's version: the client can see the server's values "as long as they can be turned into text". Functions, class instances, and closures don't cross as data — the one thing that does cross as a callable is a Server Function, which travels as a reference the client invokes over the network.
...
**Server data is borrowed, so revalidate rather than hoard it.** The `react` skill's "server state is borrowed, not owned" applies here at the framework level: a cache exists for "storing the result of data fetching ... so that future requests for the same data can be served faster", on the assumption the value will be revalidated when it goes stale. The cache is where fetched data lives; refetching it into a variable to hold onto defeats the point.
...
**The cache is the source of truth; don't copy server data into a client store.** As in the `react` skill's "never copy server data into another container", pulling fetched data into a client-side store means the copy silently stops tracking the source. Read from the server on each request and revalidate; keep the client store for the genuinely client-owned state (open/closed, theme) that the server never had.
...
**Push both boundaries as deep as the design allows.** Keep the client boundary and the dynamic boundary far down the tree, so most of the app is server-rendered and static and only the genuinely interactive or request-time leaves are isolated. Abramov's point is that the seam should move freely — place each piece "where appropriate" and "snap them together" — rather than letting one early `"use client"` or one request-time read pull the whole route across with it.
