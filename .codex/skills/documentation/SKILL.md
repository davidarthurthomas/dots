...
How to word a comment or a docs page. Each rule below names one way of writing a phrase that sounds precise while stating no fact about the system, and gives the replacement. The shared failure is writing the phrase you reached for first instead of the fact you would have had to go find.

**Name the operation.** "Carries", "holds", "takes", "handles", "deals with", "works with", and "involves" are verbs of physical possession and generic activity. Applied to data they substitute an image of a hand moving an object for the operation the code performs, and each has a precise replacement that a reader can act on. A request does not carry a user ID; it encodes the ID in the `X-User-Id` header, or includes it in the JSON body. A cache does not hold a token; it stores the token under a key until the TTL expires. A function does not take a callback; it invokes the callback once per row. That last case has a second defect: the parameter list is already in the signature, so "takes a callback" restates what the code says, and the sentence worth writing states what the function does with the callback. When several operations would fit and you cannot choose, you do not yet know which one the code performs. Read it.

**Name the mechanism, not a figure of speech for it.** Machines do not want, try, decide, know, panic, or get confused, and they are not sleepy, unhappy, or angry. Write the behavior and what triggers it: not "the worker gives up when the queue gets unhappy", but "the worker stops retrying after the fifth consecutive timeout". The same applies to idiom borrowed from speech ("kick the tires", "belt and braces", "the long pole"), which a reader outside your dialect cannot decode and a reader in translation cannot decode at all. Words that began as metaphors but now have exact technical definitions (handshake, deadlock, orphaned process, starvation, heartbeat, garbage collection, tree, queue) are the right words; use them freely. The defect is inventing a new figure of speech in place of a mechanism you did not name.

**Name the object, not the product.** "Postgres" is the name of a program. A system that uses it has a client library, a connection, a connection pool, a server process, and a cluster node, and each fails differently. "A Postgres" makes the reader guess which one you mean, so write the one you mean: "the connection pool", "the primary's server process". Rewrite "spin up a Postgres" as "start a Postgres server container". If you wrote "a" before a product name, or pluralized it, you have not yet said which object you are describing.

**Give the state as an observation.** Do not attach a state to a noun and stop there: "a saturated Postgres", "a dirty cache", "a bad connection". Say what a reader could measure to find the same state: "a pooled connection whose server did not answer within the 2s socket timeout". A state nobody can check is a guess written as a fact. If you cannot say how to observe it, you do not know it, and the comment should say what you do know.

**Use a term only where its domain defines it.** Do not borrow a term because it sounds close to what you mean. Several domains define the same word independently, each with its own meaning: "round trip" means network latency in protocol work and serialize-then-deserialize-and-compare in serialization work; "commit" is defined by version control and by transaction processing; "buffer", "stream", "handle", and "block" each have several meanings. Using a term from another domain is correct when that domain defines it the way you mean it. The defect is leaving the reader unable to tell which meaning applies, so add the words that pick one: write "round-trips through JSON" rather than a bare "round trip" in code that also makes network calls. When you cannot say which domain defines the term the way you are using it, you are guessing: use ordinary words instead.
