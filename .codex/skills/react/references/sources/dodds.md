...
Tests that reach into implementation details fail in both directions: they go red on a refactor that changed nothing a user could observe (false negative for confidence), and they can stay green while real user-facing behavior is broken (false positive). The fix is to treat the test as a stand-in for an actual user of the code (an end user interacting with rendered output, or a developer using a component's props) and never as a user of internal state or private methods. The same sentence anchors both this post and the Testing Library docs:
