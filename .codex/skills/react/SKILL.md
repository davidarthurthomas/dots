...
**Shareable state lives in the URL.** If a value should survive a refresh or a shared link (the selected tab, the active filter), it belongs in the URL, not a state variable synced to it. The router is a state manager whose store is the address bar.
...
**Every re-render starts at a state change and sweeps downward.** "Every re-render in React starts with a state change. It's the only 'trigger'"; once a component renders, "React will recursively render all child components inside of it", props or no props. Changing props can't cause a re-render the parent didn't already cause; props only start to matter once memoization is in play.
